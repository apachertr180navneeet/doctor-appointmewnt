...
<div class="container-fluid flex-grow-1 container-p-y">
    <h5 class="py-2 mb-2">
        <span class="text-primary fw-light">Education</span>
    </h5>
    <div class="row">
        <div class="col-xl-12 col-lg-12">
            <div class="card">
                <div class="card-body">
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addEducation">Add Education</button>
        ...
        <div class="modal-body">
            <div class="row">
                <div class="col mb-3">
                    ...
                    <input type="text" id="education_name" name="education_name" class="form-control" placeholder="Enter Name">
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-primary" id="addEducationsave">Save</button>
            </div>
      </div>
    </div>
  </div>
        ...
        <div class="modal-body">
            <div class="row">
                <div class="col mb-3">
                    ...
                    <label for="edit_education_name" class="form-label">Name</label>
                    <input type="text" id="edit_education_name" name="edit_education_name" class="form-control" placeholder="Enter Name">
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-primary" id="editEducationsave">Save</button>
            </div>
      </div>
    </div>
  </div>
...
<script>
    $(document).ready(function() {
        ...
        const $addSaveButton = $('#addEducationsave');
        const $editSaveButton = $('#editEducationsave');
        const $addModal = $('#addEducation');
        const $editModal = $('#editEducation');
        const $nameField = $('#education_name');
        const $editIdField = $('#edit_id');
        const $editNameField = $('#edit_education_name');

        // Define URLs and CSRF token for AJAX requests
        const urls = {
            ajax: "{{route('admin.education.alllist')}}",
            store: '{{ route("admin.education.store") }}',
            status: "{{route('admin.education.status')}}",
            delete: "{{route('admin.education.delete')}}",
            edit: "{{route('admin.education.edit')}}",
            update: "{{route('admin.education.update')}}"
        };
        ...
        $editSaveButton.click((event) => {
            ...
            $.ajax({
                type: "POST",
                url: urls.update,
                data: { _token: token, educationName: $editNameField.val(), educationId: $editIdField.val() },
                success: (response) => {
                    const messageType = response.success ? 'success' : 'error';
